...
import ConfirmRidePopup from "../components/ConfirmRidePopup"; // Import new popup

const CaptainHome = () => {
  const [ridePopupOpen, setRidePopupOpen] = useState(false);
  const [confirmRidePopupOpen, setConfirmRidePopupOpen] = useState(false); // Fullscreen details toggle
  ...
  return (
    <div className="relative h-screen bg-slate-100 overflow-hidden">
      ...
      >
        {ridePopupOpen ? "Hide Notification" : "Available Ride Notification"}
      </button>

      {/* Base Overview Dashboard Panel */}
      ...
      {/* Sliding Ride Request Notification Popup */}
      <RidePopup 
      ...
      {/* NEW: Full Screen (h-screen) Passenger Detail Card Panel overlay */}
      <ConfirmRidePopup 

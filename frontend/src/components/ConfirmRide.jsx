  ...
  return (
    <div className="mt-6 space-y-4">
      ...
      <div className="flex gap-3">
        <button onClick={() => { setSelectedRide(null); setPanelOpen(true); }} className="flex-1 rounded-3xl bg-slate-200 px-4 py-3 text-center font-semibold text-slate-900 hover:bg-slate-300 active:scale-95 transition-all">Change Ride</button>
        <button onClick={onConfirm} className="flex-1 rounded-3xl bg-black px-4 py-3 text-center font-semibold text-white hover:bg-slate-800 active:scale-95 transition-all">Confirm</button>
      </div>
    </div>
  );
};
